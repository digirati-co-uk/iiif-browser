import { createStore as create, type StoreApi } from "zustand/vanilla";
import type {
...
import type { HistoryItem } from "./IIIFBrowser.types";
  ...
  create<Store>()((set, get) => ({
    ...
    addToListing(item: HistoryItem) {
      ...
      set({
        selected: {
          ...selected,
          listing: [...listing.filter((t) => t.id !== item.id), item],
        },
      });
    },
    ...
    removeFromListing(item: HistoryItem) {
      ...
      set({
        selected: {
          ...selected,
          listing: selected.listing.filter(
        ...
        if (!status || status.loadingState === "RESOURCE_ERROR") {
          vault
            ...
            .then((loaded) => {
              if (
                loaded &&
                (resource.id !== loaded.id || resource.type !== loaded.type)
              ) {
