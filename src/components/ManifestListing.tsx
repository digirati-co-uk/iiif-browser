    ...
    <div className={$.ManifestListing} {...container}>
      ...
      >
        {sequence.sequence.map((ids, n) => (
          <div
            key={n}
            className={$.ThumbnailGroup}
            style={{ "--item-count": ids.length } as any}
          >
            {ids
                  ...
                  <CanvasSnippet
                    ...
                    onSelect={(shift) => {
                      if (shift) {
                        if (items.length) {
                          ...
                          const lastIndex = sequence.items.findIndex(
                            (t) => t.id === lastItem.id,
                          );
                    ...
                    onClick={() => {
                      if (container.ref.current) {
                        setScrollCache(
                          manifest.id,
                          container.ref.current.scrollTop,
                        );
                      }
                      select(item);
                    }}
