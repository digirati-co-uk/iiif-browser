...
interface ExplorerOutputProps {
   ...
   *   - first is default.
   */
  targets: IIIFBrowserProps["outputTargets"];
  format: OutputFormat;
...
export function useValidTargets(types: OutputType[]) {
  ...
  return useVaultSelector(() => {
    ...
    for (let i = 0; i < history.length; i++) {
      const resource: any =
        history[i]?.id === selected?.id ? selected : history[i];
...
export function ExplorerOutput(props: ExplorerOutputProps) {
  ...
  const output = mostSpecific
    ? vault.get<any>(valid[mostSpecific] as any)
    : undefined;
        ...
        {hasValid && output ? (
          <>
            <div className={$.ResourceActionDescription}>
              {output.label ? (
                <LocaleString>{output.label}</LocaleString>
              ) : (
                output.type
              )}
            </div>
