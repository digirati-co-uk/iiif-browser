...
import type {
  ...
  NormalizedCollectionItemSchemas,
...
export function CollectionListingStandalone({
  ...
  const container = useKeyboardListNavigation<HTMLDivElement>(
...
export function CollectionListing() {
  ...
  const collection = useVaultSelector<CollectionNormalized | null>(
    (state, vault) =>
      selected &&
      (selected.type === "Collection" || selected.type === "unknown")
  ...
  if (
    collectionStatus &&
    collectionStatus.loadingState === "RESOURCE_LOADING"
  ) {
    return (
      <div>
        <Spinner />
