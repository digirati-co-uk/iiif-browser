  ...
  return {
    ...
    async search() {
      await new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("External search service unavailable")),
          400,
        ),
      );
      return [];
  ...
  initialHistory: [
    {
      ...
      resource:
        "https://iiif.io/api/cookbook/recipe/0001-mvm-image/manifest.json",
      route:
        "/manifest?id=https://iiif.io/api/cookbook/recipe/0001-mvm-image/manifest.json",
    },
  ],
};
...
SearchCustomAdapterGrouped.storyName =
  "Search: Custom adapter + within (grouped)";
...
SearchInterleaved.storyName =
  "Search: Interleaved (within + external, no sections)";
            ...
            return {
              ...
              summary:
                hit.highlights
                  ...
                  .filter(Boolean)
                  .join(" · ") ?? (doc.summary ? String(doc.summary) : null),
              kind: "external",
...
SearchTypesenseConfig.storyName =
  "Search: Typesense config (requires live endpoint)";
...
function ErrorTriggerPage(): React.ReactElement {
  const [shouldCrash, setShouldCrash] = React.useState(false);
  if (shouldCrash)
    throw new Error("Simulated render crash in a page component.");
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 p-8 text-center">
      <p className="text-sm text-gray-600">
        Click the button to simulate a crash.
      </p>
      <button
        type="button"
        className="text-xs px-3 py-1.5 rounded bg-red-100 border border-red-200 text-red-700 hover:bg-red-200"
...
ErrorBoundaryTest.storyName = "Error boundary (crash on homepage)";
