import { BrowserPluginSlot } from "../browser/plugins";
...
function ManifestCardThumbnail({
  ...
  const [imgSrc] = useState(() => {
    ...
    const idHash = btoa(encodeURI(url));
  ...
  if (failed || !imgSrc) {
    return (
      <div className="w-full h-full flex items-center justify-center">
        <PortalResourceIcon type={type} className="text-4xl opacity-20" />
      </div>
    );
  }
...
export function HomepageResourceCard({
  url,
  label,
  thumbnail,
  type = "Manifest",
  ...
  const canNavigate = canResolve({ id: url, type });

  return (
    <button
      type="button"
      onClick={canNavigate ? (onOpen ?? (() => resolve(url))) : undefined}
      className={[
...
export function HomepageResourceRow({
  url,
  label,
  type = "Collection",
  onOpen,
  ...
  return (
    ...
    >
      <PortalResourceIcon type={type} className="shrink-0 text-xl" />
...
export const Homepage = () => {
  ...
  const preferredUrls = useStore(index, (state) => state.preferredResourceUrls);

  const { recentCollections, recentManifests } = useMemo(() => {
    const ids = new Set<string>(preferredUrls);
    const collections = [];
    const manifests = [];

    for (const item of history) {
      if (ids.has(item.url) || (item.resource && ids.has(item.resource)))
        continue;
      ids.add(item.url);
    ...
    <div className="h-full overflow-y-auto">
      <div className="p-6 max-w-full">
        <div className="flex flex-col gap-8">
          <BrowserPluginSlot slot="homepage" />
          {!hasHistory && !preferredUrls.length ? (
